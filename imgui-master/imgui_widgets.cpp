...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, *v ? _("[x]") : _("[ ]"));
    if (label_size.x > 0.0f)
...
bool ImGui::RadioButton(const char* label, bool active)
{
    ...
    if (g.LogEnabled)
        LogRenderedText(&total_bb.Min, active ? _("(x)") : _("( )"));
    if (label_size.x > 0.0f)
        RenderText(ImVec2(check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y), label);
...
// Getter for the old Combo() API: _("item1\0item2\0item3\0"
...
bool ImGui::Combo(const char* label, int* current_item, bool (*items_getter)(void*, int, const char**), void* data, int items_count, int popup_max_height_in_items)
{
    ...
    // The old Combo() API exposed _("popup_max_height_in_items". The new more general BeginCombo() API doesn't have/need it, but we emulate it here.
...
// Combo box helper allowing to pass all items in a single string literal holding multiple zero-terminated items _("item1\0item2\0"
...
{
    { sizeof(char),             _("%d"),   _("%d")    },  // ImGuiDataType_S8
    { sizeof(unsigned char),    _("%u"),   _("%u")    },
    { sizeof(short),            _("%d"),   _("%d")    },  // ImGuiDataType_S16
    { sizeof(unsigned short),   _("%u"),   _("%u")    },
    { sizeof(int),              _("%d"),   _("%d")    },  // ImGuiDataType_S32
    { sizeof(unsigned int),     _("%u"),   _("%u")    },
#ifdef _MSC_VER
    { sizeof(ImS64),            _("%I64d"),_("%I64d") },  // ImGuiDataType_S64
    { sizeof(ImU64),            _("%I64u"),_("%I64u") },
#else
    { sizeof(ImS64),            _("%lld"), _("%lld"  },  // ImGuiDataType_S64
    { sizeof(ImU64),            _("%llu"), _("%llu"  },
#endif
    { sizeof(float),            _("%f"),   _("%f")    },  // ImGuiDataType_Float (float are promoted to double in va_arg)
    { sizeof(double),           _("%f"),   _("%lf" )  },  // ImGuiDataType_Double
};
IM_STATIC_ASSERT(IM_ARRAYSIZE(GDataTypeInfo) == ImGuiDataType_COUNT);

// FIXME-LEGACY: Prior to 1.61 our DragInt() function internally used floats and because of this the compile-time default value for format was _("%.0f".
// Even though we changed the compile-time default, we expect users to have carried %f around, which would break the display of DragInt() calls.
// To honor backward compatibility we are rewriting the format string, unless IMGUI_DISABLE_OBSOLETE_FUNCTIONS is enabled. What could possibly go wrong?!
static const char* PatchFormatStringFloatToInt(const char* fmt)
{
    if (fmt[0] == '%' && fmt[1] == '.' && fmt[2] == '0' && fmt[3] == 'f' && fmt[4] == 0) // Fast legacy path for _("%.0f" which is expected to be the most common case.
        return _("%d");
    ...
    if (fmt_end > fmt_start && fmt_end[-1] == 'f')
    {
        ...
        return g.TempBuffer;
#else
        IM_ASSERT(0 && _("DragInt(): Invalid format string!"); // Old versions used a default parameter of _("%.0f"), please replace with e.g. _("%d"
#endif
    }
    return fmt;
}
...
bool ImGui::DataTypeApplyOpFromText(const char* buf, const char* initial_value_buf, ImGuiDataType data_type, void* p_data, const char* format)
{
    ...
    if (data_type == ImGuiDataType_S32)
    {
        ...
        if (op == '+')      { if (sscanf(buf, _("%d"), &arg1i)) *v = (int)(arg0i + arg1i); }                   // Add (use _("+-" to subtract)
        else if (op == '*') { if (sscanf(buf, _("%f"), &arg1f)) *v = (int)(arg0i * arg1f); }                   // Multiply
        else if (op == '/') { if (sscanf(buf, _("%f"), &arg1f) && arg1f != 0.0f) *v = (int)(arg0i / arg1f); }  // Divide
        else                { if (sscanf(buf, format, &arg1i) == 1) *v = arg1i; }                           // Assign constant
    }
    else if (data_type == ImGuiDataType_Float)
    {
        // For floats we have to ignore format with precision (e.g. _("%.2f") because sscanf doesn't take them in
        format = _("%f");
        ...
        if (sscanf(buf, format, &arg1f) < 1)
            return false;
        if (op == '+')      { *v = arg0f + arg1f; }                    // Add (use _("+-" to subtract)
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    else if (data_type == ImGuiDataType_S32 && strcmp(format, _("%d")) != 0) // (FIXME-LEGACY: Patch old _("%.0f" format string to use _("%d"), read function more details.)
...
bool ImGui::DragScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        value_changed |= DragScalar(_(""), data_type, p_data, v_speed, p_min, p_max, format, power);
        PopID();
        PopItemWidth();
...
//  fmt = _("blah blah"  -> return fmt
//  fmt = _("%.3f"       -> return fmt
//  fmt = _("hello %.3f" -> return fmt + 6
//  fmt = _("%.3f hello" -> return buf written with _("%.3f"
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    if (p_step != NULL)
    {
        ...
        if (InputText(_(""), buf, IM_ARRAYSIZE(buf), flags)) // PushId(label) + _("" gives us the expected ID from outside point of view
...
bool ImGui::InputScalarN(const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags)
{
    ...
    for (int i = 0; i < components; i++)
    {
        ...
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= InputScalar(_(""), data_type, p_data, p_step, p_step_fast, format, flags);
        PopID();
        PopItemWidth();
        p_data = (void*)((char*)p_data + type_size);
    }
...
// Prefer using _("const char* format" directly, which is more flexible and consistent with other API.
#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
...
// - buf_size account for the zero-terminator, so a buf_size of 6 can hold _("Hello" but not _("Hello!".
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        {
            {   _("%3d"),   _("%3d"),   _("%3d"),   _("%3d") }, // Short display
            { _("R:%3d"), _("G:%3d"), _("B:%3d"), _("A:%3d") }, // Long display for RGBA
            { _("H:%3d"), _("S:%3d"), _("V:%3d"), _("A:%3d") }  // Long display for HSVA
        };
    ...
    else if ((flags & ImGuiColorEditFlags_DisplayHex) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        if (InputText(_("##Text"), buf, IM_ARRAYSIZE(buf), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase))
        {
            ...
            i[0] = i[1] = i[2] = i[3] = 0;
            if (alpha)
                sscanf(p, _("%02X%02X%02X%02X"), (unsigned int*)&i[0], (unsigned int*)&i[1], (unsigned int*)&i[2], (unsigned int*)&i[3]); // Treat at unsigned (%X is unsigned)
            else
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSidePreview))
    {
        ...
        ColorButton(_("##current"), col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2));
        if (ref_col != NULL)
        {
            Text(_("Original"));
            ImVec4 ref_col_v4(ref_col[0], ref_col[1], ref_col[2], (flags & ImGuiColorEditFlags_NoAlpha) ? 1.0f : ref_col[3]);
            if (ColorButton(_("##original"), ref_col_v4, (flags & sub_flags_to_forward), ImVec2(square_sz * 3, square_sz * 2)))
            {
    ...
    if (!is_leaf)
    {
        ...
        if (pressed)
        {
            ...
            if ((flags & ImGuiTreeNodeFlags_OpenOnDoubleClick) && g.IO.MouseDoubleClicked[0])
                toggled = true;
            if (g.DragDropActive && is_open) // When using Drag and Drop _("hold to open" we keep the node highlighted after opening, but never close it again.
                toggled = false;
        }
...
// Tip: pass a non-visible label (e.g. _("##dummy") then you can use the space to draw other text or image.
...
// Tip: To have a list filling the entire window width, PushItemWidth(-1) and pass an non-visible label e.g. _("##empty"
...
bool ImGui::BeginMenu(const char* label, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Vertical) // (window->Flags & (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
    {
        ...
        {
            // FIXME-DPI: Values should be derived from a master _("scale" factor.
    ...
    if (!enabled) // explicitly close if an open menu becomes disabled, facilitate users code a lot in pattern such as 'if (BeginMenu(_("options"), has_object)) { ..use object.. }'
        want_close = true;
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    {
        //IMGUI_DEBUG_LOG(_("BeginTabBarEx already called this frame\n"), g.FrameCount);
        IM_ASSERT(0);
        return true;
    }
...
void    ImGui::EndTabBar()
{
    ...
    if (tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar != NULL, _("Mismatched BeginTabBar()/EndTabBar()!"));
        return;
    }
...
bool    ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags)
{
    ...
    if (tab_bar == NULL)
    {
        IM_ASSERT_USER_ERROR(tab_bar, _("BeginTabItem() Needs to be called between BeginTabBar() and EndTabBar()!"));
        return false;
    }
...
{
    // While rendering tabs, we trim 1 pixel off the top of our bounding box so they can fit within a regular frame height while looking _("detached" from it.
    ImGuiContext& g = *GImGui;
