...
/*
 ...
 - Dear ImGui is a "single pass" rasterizing implementation of the IMGUI paradigm, aimed at ease of use and high-performances.
   ...
   where the UI code is called multiple times (_("multiple passes") from a single entry point. There are pros and cons to both approaches.
 ...
 EXHIBIT 2: IMPLEMENTING CUSTOM BINDING / CUSTOM ENGINE
     ...
     while (true)
     {
        ...
        ImGui::Text(_("Hello, world!");
        MyGameUpdate(); // may use any Dear ImGui functions, e.g. ImGui::Begin(_("My window"); ImGui::Text(_("Hello, world!"); ImGui::End();
 ...
 - 2018/12/20 (1.67) - made it illegal to call Begin(_("") with an empty string. This somehow half-worked before but had various undesirable side-effects.
 ...
 A: A primer on labels and the ID Stack...
   ...
   - Unique ID are often derived from a string label:
       ...
       Button(_("OK");          // Label = "OK",     ID = hash of (..., "OK")
       Button(_("Cancel");      // Label = "Cancel", ID = hash of (..., "Cancel")
   ...
   - ID are uniquely scoped within windows, tree nodes, etc. which all pushes to the ID stack. Having
     ...
     We used "..." above to signify whatever was already pushed to the ID stack previously:
       ...
       Begin(_("MyWindow");
       Button(_("OK");          // Label = "OK",     ID = hash of (_("MyWindow", "OK")
       End();
       Begin(_("MyOtherWindow");
       Button(_("OK");          // Label = "OK",     ID = hash of (_("MyOtherWindow", "OK")
       End();

   - If you have a same ID twice in the same location, you'll have a conflict:

       Button(_("OK");
       Button(_("OK");          // ID collision! Interacting with either button will trigger the first one.
   ...
   - Solving ID conflict in a simple/local context:
     ...
     are going to be created:
       ...
       Begin(_("MyWindow");
       Button(_("Play");        // Label = "Play",   ID = hash of (_("MyWindow", "Play")
       Button(_("Play##foo1");  // Label = "Play",   ID = hash of (_("MyWindow", "Play##foo1")  // Different from above
       Button(_("Play##foo2");  // Label = "Play",   ID = hash of (_("MyWindow", "Play##foo2")  // Different from above
       End();

   - If you want to completely hide the label, but still need an ID:

       Checkbox(_("##On", &b);  // Label = "",       ID = hash of (..., "##On")   // No visible label, just a checkbox!

   - Occasionally/rarely you might want change a label while preserving a constant ID. This allows
     you to animate labels. For example you may want to include varying information in a window title bar,
     but windows are uniquely identified by their ID. Use "###" to pass a label that isn't part of ID:

       Button(_("Hello###ID");  // Label = "Hello",  ID = hash of (..., "###ID")
       Button(_("World###ID");  // Label = "World",  ID = hash of (..., "###ID")  // Same as above, even though the label looks different
   ...
   - Solving ID conflict in a more general manner:
     ...
     Begin(_("Window");
       for (int i = 0; i < 100; i++)
       {
         ...
         Button(_("Click");     // Label = "Click",  ID = hash of (_("Window", i, "Click")
         PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         ...
         PushID(obj);
         Button(_("Click");     // Label = "Click",  ID = hash of (_("Window", obj pointer, "Click")
         PopID();
       }
       for (int i = 0; i < 100; i++)
       {
         MyObject* obj = Objects[i];
         PushID(obj->Name);
         Button(_("Click");     // Label = "Click",  ID = hash of (_("Window", obj->Name, "Click")
         PopID();
       }
       End();
   ...
   - You can stack multiple prefixes into the ID stack:
       ...
       Button(_("Click");       // Label = "Click",  ID = hash of (..., "Click")
       PushID(_("node");
       Button(_("Click");       // Label = "Click",  ID = hash of (..., "node", "Click")
         PushID(my_ptr);
           Button(_("Click");   // Label = "Click",  ID = hash of (..., "node", my_ptr, "Click")
         PopID();
       PopID();
   ...
   - Tree nodes implicitly creates a scope for you by calling PushID().
       ...
       Button(_("Click");       // Label = "Click",  ID = hash of (..., "Click")
       if (TreeNode(_("node"))  // <-- this function call will do a PushID() for you (unless instructed not to, with a special flag)
       {
         Button(_("Click");     // Label = "Click",  ID = hash of (..., "node", "Click")
         TreePop();
       }
...
#if __has_warning(_("-Wzero-as-null-pointer-constant")
#pragma clang diagnostic ignored "-Wzero-as-null-pointer-constant"  // warning : zero as null pointer constant              // some standard header variations use #define NULL 0
#endif
#if __has_warning(_("-Wdouble-promotion")
...
{
    bool error = false;
    if (strcmp(version, IMGUI_VERSION) != 0) { error = true; IM_ASSERT(strcmp(version, IMGUI_VERSION) == 0 && xorstr_("Mismatched version string!")); }
    if (sz_io != sizeof(ImGuiIO)) { error = true; IM_ASSERT(sz_io == sizeof(ImGuiIO) && xorstr_("Mismatched struct layout!")); }
    if (sz_style != sizeof(ImGuiStyle)) { error = true; IM_ASSERT(sz_style == sizeof(ImGuiStyle) && xorstr_("Mismatched struct layout!")); }
    if (sz_vec2 != sizeof(ImVec2)) { error = true; IM_ASSERT(sz_vec2 == sizeof(ImVec2) && xorstr_("Mismatched struct layout!")); }
    if (sz_vec4 != sizeof(ImVec4)) { error = true; IM_ASSERT(sz_vec4 == sizeof(ImVec4) && xorstr_("Mismatched struct layout!")); }
    if (sz_vert != sizeof(ImDrawVert)) { error = true; IM_ASSERT(sz_vert == sizeof(ImDrawVert) && xorstr_("Mismatched struct layout!")); }
    if (sz_idx != sizeof(ImDrawIdx)) { error = true; IM_ASSERT(sz_idx == sizeof(ImDrawIdx) && xorstr_("Mismatched struct layout!")); }
    return !error;
}
...
ImGuiIO& ImGui::GetIO()
{
    IM_ASSERT(GImGui != NULL && xorstr_("No current context. Did you call ImGui::CreateContext() and ImGui::SetCurrentContext() ?"));
    return GImGui->IO;
}
...
static void NewFrameSanityChecks()
{
    ...
    IM_ASSERT((g.IO.DeltaTime > 0.0f || g.FrameCount == 0) && xorstr_("Need a positive DeltaTime!"));
    IM_ASSERT((g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount) && xorstr_("Forgot to call Render() or EndFrame() at the end of the previous frame?"));
    IM_ASSERT(g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && xorstr_("Invalid DisplaySize value!"));
    IM_ASSERT(g.IO.Fonts->Fonts.Size > 0 && xorstr_("Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?"));
    IM_ASSERT(g.IO.Fonts->Fonts[0]->IsLoaded() && xorstr_("Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?"));
    IM_ASSERT(g.Style.CurveTessellationTol > 0.0f && xorstr_("Invalid style setting!"));
    IM_ASSERT(g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && xorstr_("Invalid style setting. Alpha cannot be negative (allows us to avoid a few clamps in color computations)!"));
    IM_ASSERT(g.Style.WindowMinSize.x >= 1.0f && g.Style.WindowMinSize.y >= 1.0f && xorstr_("Invalid style setting."));
    IM_ASSERT(g.Style.WindowMenuButtonPosition == ImGuiDir_None || g.Style.WindowMenuButtonPosition == ImGuiDir_Left || g.Style.WindowMenuButtonPosition == ImGuiDir_Right);
    for (int n = 0; n < ImGuiKey_COUNT; n++)
        IM_ASSERT(g.IO.KeyMap[n] >= -1 && g.IO.KeyMap[n] < IM_ARRAYSIZE(g.IO.KeysDown) && xorstr_("io.KeyMap[] contains an out of bound value (need to be 0..512, or -1 for unmapped key)"));

    // Perform simple check: required key mapping (we intentionally do NOT check all keys to not pressure user into setting up everything, but Space is required and was only recently added in 1.60 WIP)
    if (g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard)
        IM_ASSERT(g.IO.KeyMap[ImGuiKey_Space] != -1 && xorstr_("ImGuiKey_Space is not mapped, required for keyboard navigation."));
...
void ImGui::UpdateDebugToolItemPicker()
{
    ...
    if (g.DebugItemPickerActive)
    {
        ...
        ImGui::BeginTooltip();
        ImGui::Text(_("HoveredId: 0x%08X"), hovered_id);
        ImGui::Text(_("Press ESC to abort picking."));
        ImGui::TextColored(GetStyleColorVec4(hovered_id ? ImGuiCol_Text : ImGuiCol_TextDisabled), _("Click to break in debugger!"));
        ImGui::EndTooltip();
    }
}
...
static void AddDrawListToDrawData(ImVector<ImDrawList*>* out_list, ImDrawList* draw_list)
{
    ...
    if (sizeof(ImDrawIdx) == 2)
        IM_ASSERT(draw_list->_VtxCurrentIdx < (1 << 16) && xorstr_("Too many vertices in ImDrawList using 16-bit indices. Read comment above"));
...
void ImGui::EndFrame()
{
    ...
    if (g.FrameCountEnded == g.FrameCount)          // Don't process EndFrame() multiple times.
        return;
    IM_ASSERT(g.WithinFrameScope && xorstr_("Forgot to call ImGui::NewFrame()?"));
...
// CalcTextSize(_("") should return ImVec2(0.0f, g.FontSize)
ImVec2 ImGui::CalcTextSize(const char* text, const char* text_end, bool hide_text_after_double_hash, float wrap_width, float font_Size)
{
    ...
    float font_size_t;
    if (font_Size == 0.0f)
        font_size_t = g.FontSize;
    else
        font_size_t = font_Size;

    if (text == text_display_end)
        return ImVec2(0.0f, font_size_t);

    ImVec2 text_size = font->CalcTextSizeA(font_size_t, FLT_MAX, wrap_width, text, text_display_end, NULL);
...
static bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    char title[256];
    if (name)
        ImFormatString(title, IM_ARRAYSIZE(title), xorstr_("%s/%s_%08X"), parent_window->Name, name, id);
    else
        ImFormatString(title, IM_ARRAYSIZE(title), xorstr_("%s/%08X"), parent_window->Name, id);
...
{
    ImGuiContext& g = *GImGui;
    //IMGUI_DEBUG_LOG(_("CreateNewWindow '%s', flags = 0x%08X\n", name, flags);
...
void ImGui::RenderWindowTitleBarContents(ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open)
{
    ...
    const char* UNSAVED_DOCUMENT_MARKER = xorstr_("*");
...
void ImGui::End()
{
    ...
    {
        IM_ASSERT_USER_ERROR(g.CurrentWindowStack.Size > 1, xorstr_("Calling End() too many times!"));
        return;
    }
    ...
    if (window->Flags & ImGuiWindowFlags_ChildWindow)
        IM_ASSERT_USER_ERROR(g.WithinEndChild, xorstr_("Must call EndChild() and not End()!"));
...
void ImGui::FocusWindow(ImGuiWindow* window)
{
    ...
    if (g.NavWindow != window)
    {
        ...
        //IMGUI_DEBUG_LOG(_("FocusWindow(\"%s\")\n", window ? window->Name : NULL);
    }
...
void ImGui::PushStyleVar(ImGuiStyleVar idx, float val)
{
    ...
    IM_ASSERT(0 && xorstr_("Called PushStyleVar() float variant but variable is not a float!"));
}
...
const char* ImGui::GetStyleColorName(ImGuiCol idx)
{
    ...
    switch (idx)
    {
    case ImGuiCol_Text: return                xorstr_(  "Text");
    case ImGuiCol_TextDisabled: return        xorstr_(     "TextDisabled");
    case ImGuiCol_WindowBg: return            xorstr_(       "WindowBg");
    case ImGuiCol_ChildBg: return             xorstr_(  "ChildBg");
    case ImGuiCol_PopupBg: return             xorstr_("PopupBg");
    case ImGuiCol_Border: return              xorstr_(      "Border");
    case ImGuiCol_BorderShadow: return        xorstr_(  "BorderShadow");
    case ImGuiCol_FrameBg: return             xorstr_( "FrameBg");
    case ImGuiCol_FrameBgHovered: return      xorstr_(  "FrameBgHovered");
    case ImGuiCol_FrameBgActive: return       xorstr_( "FrameBgActive");
    case ImGuiCol_TitleBg: return             xorstr_(  "TitleBg");
    case ImGuiCol_TitleBgActive: return       xorstr_(  "TitleBgActive");
    case ImGuiCol_TitleBgCollapsed: return    xorstr_(  "TitleBgCollapsed");
    case ImGuiCol_MenuBarBg: return           xorstr_( "MenuBarBg");
    case ImGuiCol_ScrollbarBg: return         xorstr_(     "ScrollbarBg");
    case ImGuiCol_ScrollbarGrab: return       xorstr_(     "ScrollbarGrab");
    case ImGuiCol_ScrollbarGrabHovered: return xorstr_(     "ScrollbarGrabHovered");
    case ImGuiCol_ScrollbarGrabActive: return xorstr_(      "ScrollbarGrabActive");
    case ImGuiCol_CheckMark: return           xorstr_(         "CheckMark");
    case ImGuiCol_SliderGrab: return          xorstr_( "SliderGrab");
    case ImGuiCol_SliderGrabActive: return    xorstr_( "SliderGrabActive");
    case ImGuiCol_Button: return              xorstr_(     "Button");
    case ImGuiCol_ButtonHovered: return       xorstr_( "ButtonHovered");
    case ImGuiCol_ButtonActive: return        xorstr_(  "ButtonActive");
    case ImGuiCol_Header: return              xorstr_(     "Header");
    case ImGuiCol_HeaderHovered: return       xorstr_(         "HeaderHovered");
    case ImGuiCol_HeaderActive: return        xorstr_(  "HeaderActive");
    case ImGuiCol_Separator: return           xorstr_(  "Separator");
    case ImGuiCol_SeparatorHovered: return    xorstr_( "SeparatorHovered");
    case ImGuiCol_SeparatorActive: return     xorstr_(  "SeparatorActive");
    case ImGuiCol_ResizeGrip: return          xorstr_( "ResizeGrip");
    case ImGuiCol_ResizeGripHovered: return   xorstr_( "ResizeGripHovered");
    case ImGuiCol_ResizeGripActive: return    xorstr_(  "ResizeGripActive");
    case ImGuiCol_Tab: return                 xorstr_(     "Tab");
    case ImGuiCol_TabHovered: return          xorstr_( "TabHovered");
    case ImGuiCol_TabActive: return           xorstr_( "TabActive");
    case ImGuiCol_TabUnfocused: return        xorstr_(  "TabUnfocused");
    case ImGuiCol_TabUnfocusedActive: return  xorstr_(     "TabUnfocusedActive");
    case ImGuiCol_PlotLines: return           xorstr_(   "PlotLines");
    case ImGuiCol_PlotLinesHovered: return    xorstr_(     "PlotLinesHovered");
    case ImGuiCol_PlotHistogram: return       xorstr_(     "PlotHistogram");
    case ImGuiCol_PlotHistogramHovered: return xorstr_(          "PlotHistogramHovered");
    case ImGuiCol_TextSelectedBg: return      xorstr_(     "TextSelectedBg");
    case ImGuiCol_DragDropTarget: return      xorstr_(     "DragDropTarget");
    case ImGuiCol_NavHighlight: return        xorstr_( "NavHighlight");
    case ImGuiCol_NavWindowingHighlight: return xorstr_( "NavWindowingHighlight");
    case ImGuiCol_NavWindowingDimBg: return   xorstr_( "NavWindowingDimBg");
    case ImGuiCol_ModalWindowDimBg: return    xorstr_( "ModalWindowDimBg");
    }
    IM_ASSERT(0);
...
void ImGui::OpenPopupEx(ImGuiID id)
{
    ...
    //IMGUI_DEBUG_LOG(_("OpenPopupEx(0x%08X)\n", g.FrameCount, id);
...
void ImGui::ClosePopupsOverWindow(ImGuiWindow* ref_window, bool restore_focus_to_window_under_popup)
{
    ...
    {
        //IMGUI_DEBUG_LOG(_("ClosePopupsOverWindow(%s) -> ClosePopupToLevel(%d)\n", ref_window->Name, popup_count_to_keep);
...
void ImGui::CloseCurrentPopup()
{
    ...
    //IMGUI_DEBUG_LOG(_("CloseCurrentPopup %d -> %d\n", g.BeginPopupStack.Size - 1, popup_idx);
...
void ImGui::NavInitWindow(ImGuiWindow* window, bool force_reinit)
{
    ...
    if (!(window->Flags & ImGuiWindowFlags_NoNavInputs))
        if (!(window->Flags & ImGuiWindowFlags_ChildWindow) || (window->Flags & ImGuiWindowFlags_Popup) || (window->NavLastIds[0] == 0) || force_reinit)
            init_for_nav = true;
    //IMGUI_DEBUG_LOG(_("[Nav] NavInitWindow() init_for_nav=%d, window=\"%s\", layer=%d\n", init_for_nav, window->Name, g.NavLayer);
    if (init_for_nav)
    {
...
#if 0
    if (g.NavScoringCount > 0) IMGUI_DEBUG_LOG(_("NavScoringCount %d for '%s' layer %d (Init:%d, Move:%d)\n", g.FrameCount, g.NavScoringCount, g.NavWindow ? g.NavWindow->Name : "NULL", g.NavLayer, g.NavInitRequest || g.NavInitResultId != 0, g.NavMoveRequest);
#endif
...
#undef NAV_MAP_KEY
    ...
    if (g.NavInitResultId != 0 && (!g.NavDisableHighlight || g.NavInitRequestFromMove) && g.NavWindow)
    {
        ...
        //IMGUI_DEBUG_LOG(_("[Nav] Apply NavInitRequest result: 0x%08X Layer %d in \"%s\"\n", g.NavInitResultId, g.NavLayer, g.NavWindow->Name);
    ...
    {
        //IMGUI_DEBUG_LOG(_("[Nav] NavInitRequest from move, window \"%s\", layer=%d\n", g.NavWindow->Name, g.NavLayer);
...
void ImGui::NavUpdateWindowingOverlay()
{
    ...
    if (g.NavWindowingList == NULL)
        g.NavWindowingList = FindWindowByName(_("###NavWindowingList"));
    ...
    for (;;)
    {
        ...
        if (!is_last_line || (line_start != line_end))
        {
            ...
            if (log_new_line || !is_first_line)
                LogText(_("%*s%.*s"), tree_depth * 4, "", char_count, line_start);
            else if (g.LogLineFirstItem)
                LogText(_("%*s%.*s"), tree_depth * 4, "", char_count, line_start);
            else
                LogText(_(" %.*s"), char_count, line_start);
...
void ImGui::LogButtons()
{
    ...
    SliderInt(_("Default Depth"), &g.LogDepthToExpandDefault, 0, 9, NULL);
...
static void SetClipboardTextFn_DefaultImpl(void*, const char* text)
{
    ...
    if (cf_data)
    {
        PasteboardPutItemFlavor(main_clipboard, (PasteboardItemID)1, CFSTR(_("public.utf8-plain-text"), cf_data, 0);
        CFRelease(cf_data);
    }
}
...
static const char* GetClipboardTextFn_DefaultImpl(void*)
{
    ...
    for (ItemCount i = 0; i < item_count; i++)
    {
        ...
        {
            CFDataRef cf_data;
            if (PasteboardCopyItemFlavorData(main_clipboard, item_id, CFSTR(_("public.utf8-plain-text"), &cf_data) == noErr)
            {
...
{
    if (!ImGui::Begin(_("Dear ImGui Metrics"), p_open))
    {
        ImGui::End();
        return;
    }
    ...
    ImGui::Text(_("Application average %.3f ms/frame (%.1f FPS)"), 1000.0f / io.Framerate, io.Framerate);
    ImGui::Text(_("%d vertices, %d indices (%d triangles)"), io.MetricsRenderVertices, io.MetricsRenderIndices, io.MetricsRenderIndices / 3);
    ImGui::Text(_("%d active windows (%d visible)"), io.MetricsActiveWindows, io.MetricsRenderWindows);
    ImGui::Text(_("%d active allocations"), io.MetricsActiveAllocations);
    ImGui::Separator();
    ...
    struct Funcs
    {
        ...
        static void NodeDrawList(ImGuiWindow* window, ImDrawList* draw_list, const char* label)
        {
            ...
            if (window && !window->WasActive)
                ImGui::TextDisabled(_("Warning: owning Window is inactive. This DrawList is not being rendered!"));
            ...
            for (const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++)
            {
                ...
                char buf[300];
                ImFormatString(buf, IM_ARRAYSIZE(buf), _("DrawCmd: %4d triangles, Tex 0x%p, ClipRect (%4.0f,%4.0f)-(%4.0f,%4.0f)"),
                ...
                ImFormatString(buf, IM_ARRAYSIZE(buf), _("Mesh: ElemCount: %d, VtxOffset: +%d, IdxOffset: +%d, Area: ~%0.f px"), pcmd->ElemCount, pcmd->VtxOffset, pcmd->IdxOffset, total_area);
        ...
        {
            if (!ImGui::TreeNode((void*)(uintptr_t)columns->ID, _("Columns Id: 0x%08X, Count: %d, Flags: 0x%04X"), columns->ID, columns->Count, columns->Flags))
                return;
            ImGui::BulletText(_("Width: %.1f (MinX: %.1f, MaxX: %.1f)"), columns->OffMaxX - columns->OffMinX, columns->OffMinX, columns->OffMaxX);
            for (int column_n = 0; column_n < columns->Columns.Size; column_n++)
                ImGui::BulletText(_("Column %02d: OffsetNorm %.3f (= %.1f px)"), column_n, columns->Columns[column_n].OffsetNorm, GetColumnOffsetFromNorm(columns, columns->Columns[column_n].OffsetNorm));
            ImGui::TreePop();
        }
        ...
        static void NodeWindow(ImGuiWindow* window, const char* label)
        {
            if (window == NULL)
            {
                ImGui::BulletText(_("%s: NULL"), label);
                return;
            }
            bool open = ImGui::TreeNode(window, _("%s '%s', %d @ 0x%p"), label, window->Name, (window->Active || window->WasActive), window);
            ...
            ImGui::BulletText(_("Pos: (%.1f,%.1f), Size: (%.1f,%.1f), ContentSize (%.1f,%.1f)"), window->Pos.x, window->Pos.y, window->Size.x, window->Size.y, window->ContentSize.x, window->ContentSize.y);
            ImGui::BulletText(_("Flags: 0x%08X (%s%s%s%s%s%s%s%s%s..)"), flags,
                (flags & ImGuiWindowFlags_ChildWindow) ? "Child " : "", (flags & ImGuiWindowFlags_Tooltip) ? "Tooltip " : "", (flags & ImGuiWindowFlags_Popup) ? "Popup " : "",
                (flags & ImGuiWindowFlags_Modal) ? "Modal " : "", (flags & ImGuiWindowFlags_ChildMenu) ? "ChildMenu " : "", (flags & ImGuiWindowFlags_NoSavedSettings) ? "NoSavedSettings " : "",
                (flags & ImGuiWindowFlags_NoMouseInputs) ? "NoMouseInputs" : "", (flags & ImGuiWindowFlags_NoNavInputs) ? "NoNavInputs" : "", (flags & ImGuiWindowFlags_AlwaysAutoResize) ? "AlwaysAutoResize" : "");
            ImGui::BulletText(_("Scroll: (%.2f/%.2f,%.2f/%.2f) Scrollbar:%s%s"), window->Scroll.x, window->ScrollMax.x, window->Scroll.y, window->ScrollMax.y, window->ScrollbarX ? "X" : "", window->ScrollbarY ? "Y" : "");
            ImGui::BulletText(_("Active: %d/%d, WriteAccessed: %d, BeginOrderWithinContext: %d"), window->Active, window->WasActive, window->WriteAccessed, (window->Active || window->WasActive) ? window->BeginOrderWithinContext : -1);
            ImGui::BulletText(_("Appearing: %d, Hidden: %d (CanSkip %d Cannot %d), SkipItems: %d"), window->Appearing, window->Hidden, window->HiddenFramesCanSkipItems, window->HiddenFramesCannotSkipItems, window->SkipItems);
            ImGui::BulletText(_("NavLastIds: 0x%08X,0x%08X, NavLayerActiveMask: %X"), window->NavLastIds[0], window->NavLastIds[1], window->DC.NavLayerActiveMask);
            ImGui::BulletText(_("NavLastChildNavWindow: %s"), window->NavLastChildNavWindow ? window->NavLastChildNavWindow->Name : "NULL");
            if (!window->NavRectRel[0].IsInverted())
                ImGui::BulletText(_("NavRectRel[0]: (%.1f,%.1f)(%.1f,%.1f)"), window->NavRectRel[0].Min.x, window->NavRectRel[0].Min.y, window->NavRectRel[0].Max.x, window->NavRectRel[0].Max.y);
            else
                ImGui::BulletText(_("NavRectRel[0]: <None>"));
            if (window->RootWindow != window) NodeWindow(window->RootWindow, _("RootWindow"));
            if (window->ParentWindow != NULL) NodeWindow(window->ParentWindow, _("ParentWindow"));
            if (window->DC.ChildWindows.Size > 0) NodeWindows(window->DC.ChildWindows, _("ChildWindows"));
            if (window->ColumnsStorage.Size > 0 && ImGui::TreeNode(_("Columns"), _("Columns sets (%d)"), window->ColumnsStorage.Size))
            {
                for (int n = 0; n < window->ColumnsStorage.Size; n++)
                    NodeColumns(&window->ColumnsStorage[n]);
                ImGui::TreePop();
            }
            NodeStorage(&window->StateStorage, _("Storage"));
            ImGui::TreePop();
        }
        ...
        static void NodeTabBar(ImGuiTabBar* tab_bar)
        {
            ...
            ImFormatString(p, buf_end - p, _("TabBar (%d tabs)%s"), tab_bar->Tabs.Size, (tab_bar->PrevFrameVisible < ImGui::GetFrameCount() - 2) ? _(" *Inactive*") : _(""));
            if (ImGui::TreeNode(tab_bar, _("%s"), buf))
            {
                for (int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++)
                {
                    const ImGuiTabItem* tab = &tab_bar->Tabs[tab_n];
                    ImGui::PushID(tab);
                    if (ImGui::SmallButton(_("<"))) { TabBarQueueChangeTabOrder(tab_bar, tab, -1); } ImGui::SameLine(0, 2);
                    if (ImGui::SmallButton(_(">"))) { TabBarQueueChangeTabOrder(tab_bar, tab, +1); } ImGui::SameLine();
                    ImGui::Text(_("%02d%c Tab 0x%08X '%s'"), tab_n, (tab->ID == tab_bar->SelectedTabId) ? '*' : ' ', tab->ID, (tab->NameOffset != -1) ? tab_bar->GetTabName(tab) : "");
                    ImGui::PopID();
                }
                ImGui::TreePop();
            }
        ...
        {
            if (!ImGui::TreeNode(label, _("%s: %d entries, %d bytes"), label, storage->Data.Size, storage->Data.size_in_bytes()))
                return;
            for (int n = 0; n < storage->Data.Size; n++)
            {
                const ImGuiStorage::ImGuiStoragePair& p = storage->Data[n];
                ImGui::BulletText(_("Key 0x%08X Value { i: %d }"), p.key, p.val_i); // Important: we currently don't store a type, real value may not be integer.
            }
            ImGui::TreePop();
        }
    };
    ...
    if (ImGui::TreeNode(_("DrawLists"), _("Active DrawLists (%d)"), g.DrawDataBuilder.Layers[0].Size))
    {
    ...
    if (ImGui::TreeNode(_("Popups"), _("Popups (%d)"), g.OpenPopupStack.Size))
    {
        for (int i = 0; i < g.OpenPopupStack.Size; i++)
        {
            ImGuiWindow* window = g.OpenPopupStack[i].Window;
            ImGui::BulletText(_("PopupID: %08x, Window: '%s'%s%s"), g.OpenPopupStack[i].PopupId, window ? window->Name : _("NULL"), window && (window->Flags & ImGuiWindowFlags_ChildWindow) ? _(" ChildWindow") : "", window && (window->Flags & ImGuiWindowFlags_ChildMenu) ? _(" ChildMenu") : "");
        }
        ImGui::TreePop();
    }

    if (ImGui::TreeNode(_("TabBars"), _("Tab Bars (%d)"), g.TabBars.GetSize()))
    {
...
#if 0
    if (ImGui::TreeNode(_("Tables", "Tables (%d)", g.Tables.GetSize()))
    {
        ImGui::TreePop();
    }
#endif

    if (ImGui::TreeNode(_("Internal state")))
    {
        const char* input_source_names[] = { "None", "Mouse", "Nav", "NavKeyboard", "NavGamepad" }; IM_ASSERT(IM_ARRAYSIZE(input_source_names) == ImGuiInputSource_COUNT);
            ...
            ImGui::Text(_("HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d"), g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
        ImGui::Text(_("ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, Source: %s"), g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer, g.ActiveIdAllowOverlap, input_source_names[g.ActiveIdSource]);
        ImGui::Text(_("ActiveIdWindow: '%s'"), g.ActiveIdWindow ? g.ActiveIdWindow->Name : _("NULL"));
        ImGui::Text(_("MovingWindow: '%s'"), g.MovingWindow ? g.MovingWindow->Name : _("NULL"));
        ImGui::Text(_("NavWindow: '%s'"), g.NavWindow ? g.NavWindow->Name : _("NULL"));
        ImGui::Text(_("NavId: 0x%08X, NavLayer: %d"), g.NavId, g.NavLayer);
        ImGui::Text(_("NavInputSource: %s"), input_source_names[g.NavInputSource]);
        ImGui::Text(_("NavActive: %d, NavVisible: %d"), g.IO.NavActive, g.IO.NavVisible);
        ImGui::Text(_("NavActivateId: 0x%08X, NavInputId: 0x%08X"), g.NavActivateId, g.NavInputId);
        ImGui::Text(_("NavDisableHighlight: %d, NavDisableMouseHover: %d"), g.NavDisableHighlight, g.NavDisableMouseHover);
        ImGui::Text(_("NavWindowingTarget: '%s'"), g.NavWindowingTarget ? g.NavWindowingTarget->Name : _("NULL"));
    ...
    if (ImGui::TreeNode(_("Tools")))
    {
        // The Item Picker tool is super useful to visually select an item and break into the call-stack of where it was submitted.
        if (ImGui::Button(_("Item Picker..")))
            ImGui::DebugStartItemPicker();
        ImGui::SameLine();
        MetricsHelpMarker(_("Will call the IM_DEBUG_BREAK() macro to break in debugger.\nWarning: If you don't have a debugger attached, this will probably crash."));

        ImGui::Checkbox(_("Show windows begin order"), &show_windows_begin_order);
        ImGui::Checkbox(_("Show windows rectangles"), &show_windows_rects);
        ImGui::SameLine();
        ImGui::SetNextItemWidth(ImGui::GetFontSize() * 12);
        show_windows_rects |= ImGui::Combo(_("##show_windows_rect_type"), &show_windows_rect_type, wrt_rects_names, WRT_Count);
        if (show_windows_rects && g.NavWindow)
        {
            ImGui::BulletText(_("'%s':"), g.NavWindow->Name);
            ImGui::Indent();
            for (int rect_n = 0; rect_n < WRT_Count; rect_n++)
            {
                ImRect r = Funcs::GetWindowRect(g.NavWindow, rect_n);
                ImGui::Text(_("(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) %s"), r.Min.x, r.Min.y, r.Max.x, r.Max.y, r.GetWidth(), r.GetHeight(), wrt_rects_names[rect_n]);
            }
            ImGui::Unindent();
        }
        ImGui::Checkbox(_("Show details when hovering ImDrawCmd node"), &show_drawcmd_details);
        ImGui::TreePop();
    }
